...
# Code by Jonas Schröder with modifications
...
extractInfo <- function(index){
  ...
  for(i in 1:maxrows){
    if(i == maxrows){
      ...
      assign("post_type", post_type, envir = .GlobalEnv)
      getNewPosts(index)
    } else {
      ...
      post_comments[index] <-posts[i, 6]
      post_type [index] <-posts[i, 2]
...
#-----------------------------
table <- do.call(rbind.data.frame, Map('c', post_id, post_url, post_img_url, post_likes, post_owner, post_text, post_time, post_comments, post_type))
colnames(table) <- c("ID", "Post_URL", "Img_URL", "Likes", "Owner", "Text", "Date", "Comments", "Type")
time <- Sys.time()
